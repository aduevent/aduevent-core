...
<head>
    ...
    $activePage = "studentSearchResult"; ?>
</head>
            ...
            echo "<div style='display: flex; justify-content: center;'>
                    <table style='width: 60%; border-collapse: collapse; margin-top: 20px;'>
                        ...
                        if ($events_result->num_rows > 0) {
                            ...
                            while ($event = $events_result->fetch_assoc()) {
                                ...
                                $buttonLink = ($event['eventStatus'] == 1) ? "studentEventDetails.php?eventID=" . $event['eventID'] : "approverDocumentViewing2.php?eventID=" . $event['eventID'];

                                echo "<tr style='border-bottom: 1px solid #ddd;'>
                                        <td style='padding: 10px;'>" . $event['eventTitle'] . " - " . $organization['organizationName'] . "</td>
                                        <td style='text-align: right; padding: 10px;'>
                                        <form action='$buttonLink' method='POST'>
                                            <input type='hidden' name='eventID' value='" . $event['eventID'] . "'>
                                            <button type='submit' class='btn btn-primary' style='border-radius: 50%; padding: 10px 12px;'><i class='bi bi-eye'></i></button>
                                        </form>
                                        </td>
                                    </tr>";
                            }
                    ...
                    if ($orgs_result->num_rows > 0) {
                        ...
                        while ($org = $orgs_result->fetch_assoc()) {
                            $buttonLink = "studentRsoList.php?organizationID=" . $org['organizationID'];

                            echo "<tr style='border-bottom: 1px solid #ddd;'>
                                    <td style='padding: 10px;'>" . $org['organizationName'] . "</td>
                                    <td style='text-align: right; padding: 10px;'>
                                        <a href='$buttonLink'><button class='btn btn-primary' style='border-radius: 50%; padding: 10px 12px; background-color: #000080; border: none;'><i class='bi bi-eye'></i></button></a>
                                    </td>
                                </tr>";
                        }
                    }
