...
// Assuming you're getting organizationID from the button click as well
...
// Calculate the academic year and set the appropriate start and end dates
...
$sql = "SELECT pointSystemCategoryID, COUNT(*) AS eventCount FROM event 
        WHERE organizationID = ? AND eventDate BETWEEN ? AND ? AND eventStatus = '1'
        GROUP BY pointSystemCategoryID";
$stmt = $conn->prepare($sql);
$stmt->bind_param("iss", $orgId, $academicYearStartDate, $academicYearEndDate);
$stmt->execute();
$result = $stmt->get_result();

$eventCounts = array_fill(1, 5, 0);
if ($result) {
...
$registrationSql = "SELECT e.eventID, e.eventTitle, COUNT(er.eventID) AS registrationCount 
                    ...
                    WHERE e.organizationID = ? AND e.eventDate BETWEEN ? AND ? AND eventStatus = '1'
...
function showEventDetails(categoryID) {
    ...
    var orgId = "<?= $orgId ?>";  // Use PHP to pass the orgId from the button
    xhr.open("GET", "osaFetchEvents.php?categoryID=" + categoryID + "&organizationID=" + orgId + "&academicYear=<?= $defaultAcademicYear ?>", true);
    xhr.onload = function () {
        if (this.status === 200) {
