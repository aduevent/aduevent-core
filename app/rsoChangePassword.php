...
if (isset($_POST['verify_password_otp'])) {
    ...
    if ($enteredOtp == $_SESSION['change_password_otp']) {
        ...
        $query = "UPDATE studentuser SET password = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("si", $hashedPassword, $userID);
        if ($stmt->execute()) {
            echo "<script>
                    alert('Password changed successfully.');
                    window.location.href = 'rsoIndex.php';
                  </script>";
        } else {
    ...
    window.location.href = 'rsoProfileViewing.php?otpSent=true';
</script>";
    }
}
elseif (isset($_POST['change_password'])) {
    ...
    $query = "SELECT password FROM studentuser WHERE id = ?";
        $stmt = $conn->prepare($query);
    ...
    if ($newPassword === $confirmNewPassword) {
        ...
        if ($recipientEmail) {
            ...
            try {
                ...
                $mail->send();
                header("Location: rsoProfileViewing.php?otpSent=true");
        exit;
            } catch (Exception $e) {
